...
let
  ...
  mkServerCommand = name: serverCfg:
    let
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services = lib.mapAttrs' (name: serverCfg:
      lib.nameValuePair "minecraft-${name}" (lib.mkIf serverCfg.enable {
    ...
    networking.firewall = {
      allowedTCPPorts = lib.flatten (lib.mapAttrsToList (_: s:
        lib.optional s.openFirewall s.port
      ) enabledServers);
      allowedUDPPorts = lib.flatten (lib.mapAttrsToList (_: s:
        lib.optional s.openFirewall s.port
      ) enabledServers);
    };
