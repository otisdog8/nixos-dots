# Off-site borg backups for constitution (borgbase), replacing the old
# git-crypt-era secrets.nix (which carried a plaintext passphrase and never
# actually enabled the service). Both the passphrase and the repo URL are
# managed by sops-nix so neither lands in the world-readable nix store:
# borgmatic reads the passphrase at runtime via encryption_passcommand, and
# the repo path is a ''${BORG_REPO} placeholder that borgmatic interpolates
# from the environment — supplied to the systemd unit as an EnvironmentFile
# drop-in.
#
# sops base config (defaultSopsFile + host age key) lives in ./default.nix;
# this file only declares the secrets it consumes.
#
# ── One-time bootstrap on constitution ───────────────────────────────────────
