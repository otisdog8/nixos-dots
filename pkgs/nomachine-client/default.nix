...
stdenv.mkDerivation rec {
  ...
  # Wrappers reference $out/NX/bin/*.bin (not $out/bin/*.bin) so nixpak's
  # grep-based "${app}/bin/nxplayer" path substitution doesn't substring-match
  # the wrapper's "${app}/bin/nxplayer.bin" reference and pull a stale copy
  # into the sandbox overrides — which would conflict with nixpak's own
  # launcher at $out/bin/nxplayer.
  postFixup = ''
    makeWrapper $out/NX/bin/nxplayer.bin $out/bin/nxplayer --set NX_SYSTEM $out/NX
    makeWrapper $out/NX/bin/nxrunner.bin $out/bin/nxrunner --set NX_SYSTEM $out/NX
